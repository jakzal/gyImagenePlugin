...
/**
 * gyThumbnail provides a mechanism for creating thumbnail images.
 *
...
class gyThumbnail
{
  ...
  public function __construct($maxWidth = null, $maxHeight = null, $scale = true, $inflate = true, $quality = 75, $adapterClass = null, $adapterOptions = array())
  {
    if (!$adapterClass)
    {
      ...
      {
        $adapterClass = 'gyGDAdapter';
      }
      else
      {
        $adapterClass = 'gyImageMagickAdapter';
      }
    }
  ...
  public function loadFile($image)
  {
    if (preg_match('#^http(s)?://#', $image))
    {
      if (class_exists('sfWebBrowser'))
      {
        ...
        $this->tempFile = tempnam('/tmp', 'gyThumbnailPlugin');
